Router.configure({
  ...
  waitOn: function() {
    return [
      ...
      Meteor.subscribe("categoryFields"),
...
Router.route('/lostpwd', {name: 'lostPass', layoutTemplate: 'layoutBlank'});
Router.route('/changePassword/:_id', {name: 'resetPassword', layoutTemplate: 'layoutBlank'});
Router.route('/generateReport', {name: 'generateGRI'});
Router.route('/actionList', {name: 'actionSetupList'});
...
Router.route('/gri/:startDate/:endDate', {name: 'griReports'});
...
var setActionList = function() {
  Session.set("currentMenu", "ACTIONSETUP");
}
...
Router.onAfterAction(setActionList, {only: 'actionSetupList'});

Router.onAfterAction(setClearMenu, {except: ['plan', 'sms', 'myActionList', 'reports', 'userList', 'referenceDataList', 'dashboard', 'intents', 'profileItem', 'adminSettings', 'organisation', 'feedbackList', 'staffList', 'actionSetupList']});
